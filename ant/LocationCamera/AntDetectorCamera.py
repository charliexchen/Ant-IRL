...
class AntDetectorCamera:
    """
    This class abstracts away all of the openCV magic so all we're left with a reliable position and orientation data of
    the robot.

    We have one aruco marker (QR code's diminutive friend) on the robot and 4 on the ground. We can use
    openCV to detect them, and the use a perspective transform to rotate and skew the image to a rectangle of predefined
    dimensions.

    Also, contains some magic to correct the fisheye lens effect on the cheap webcam I'm using.
    """
    ...
    def __init__(self, calibration_data_path="LocationCamera/calib.npz"):
        ...
        self.arena_bounds, self.ant_centre, self.ant_front, self.ant_orientation_vector = (
            None,
            None,
            None,
            None,
    ...
    def ant_orientation_angle(self):
        if self.ant_orientation_vector is None:
            return None
        return np.arctan2(
            self.ant_orientation_vector[0], self.ant_orientation_vector[1]
        )

    def _detect_location_and_orentation(self):
        marker_corners = np.array([self.marker_dict[self.Ant_MARKER_ID]])
        corrected_marker_corners = cv2.perspectiveTransform(
            marker_corners, self.perspective_matrix
        )[0]
        self.ant_centre = self._get_centre(corrected_marker_corners)
        self.ant_front = (
            corrected_marker_corners[0] + corrected_marker_corners[3]
        ) / 2  # centre of the front left and front right corner of marker
        unnormalised_orientation_vector = self.ant_front - self.ant_centre
        self.ant_orientation_vector = unnormalised_orientation_vector / np.linalg.norm(
            unnormalised_orientation_vector
        )
    ...
    def _get_arena_marker_centres(self):
        ...
        for id, marker_corners in self.marker_dict.items():
            if id in self.ARENA_CORNER_MARKER_ID_MAP:
                arena_marker_points[
                    self.ARENA_CORNER_MARKER_ID_MAP[id]
                ] = self._get_centre(marker_corners)
    ...
    def _get_arena_bounding_box(self):
        ...
        self.arena_bounds = arena_centre + (
            arena_marker_points - arena_centre
        ) * np.array([self.X_SCALE, self.Y_SCALE])
    ...
    def _get_marker_dict_and_annotated_frame(self, img):
        ...
        corners, ids, _rejected_points = aruco.detectMarkers(
    ...
    def _set_perspective_matrix(self, x_border=0, y_border=0):
        dst = np.array(
            ...
            dtype="float32",
        ) + np.array([[y_border, x_border]], dtype="float32")
        # compute the perspective transform matrix and then apply it
        self.perspective_matrix = cv2.getPerspectiveTransform(
            self.arena_bounds.astype(np.float32), dst
        )
    ...
    def correct_and_annotate_frame(self, image):
        ...
        self.marker_dict, annotated_image = self._get_marker_dict_and_annotated_frame(
            undistorted_image
        )
        ...
        if self.arena_bounds is not None:
            cv2.polylines(
                annotated_image,
                [self.arena_bounds.astype(int).reshape((-1, 1, 2))],
                True,
                (255, 0, 0),
                2,
            )
        ...
        if self._is_ant_detected():
            ...
            cv2.polylines(
                corrected_image,
                [pointer.astype(int).reshape((-1, 1, 2))],
                True,
                (0, 255, 0),
                5,
            )
        if self.target is not None:
            cv2.circle(
                corrected_image, tuple(self.target.astype(int)), 20, (0, 0, 255), 3
            )
        corrected_image = self.add_reward_boundaries_and_comments(corrected_image)
        return corrected_image

    def add_reward_boundaries_and_comments(self, image):
        """
        Draw some boxes to indicate reward/death area.
        clean this up later.
        """
        cv2.rectangle(image, (0, 0), (int(0.8 * 575), 95), (0, 0, 255), 2)
        cv2.rectangle(image, (0, 385), (int(0.8 * 575), 480), (0, 0, 255), 2)
        cv2.rectangle(image, (int(0.8 * 575), 0), (575, 480), (0, 255, 0), 3)
        if self.text is not None:
            ...
            thickness = 1
            image = cv2.putText(
                image, self.text, org, font, fontScale, color, thickness, cv2.LINE_AA
            )
        return image
...
if __name__ == "__main__":
    ...
    cv2.imshow("Processed vs unprocessed", frame)
    cv2.setMouseCallback(
        "Processed vs unprocessed", locator.initialize_mouse_handler, locator
    )
    while True:
        _, frame = locator.cap.read()
        annotated_frame = locator.correct_and_annotate_frame(frame)
        cv2.imshow("Processed vs unprocessed", np.hstack((annotated_frame, frame)))
