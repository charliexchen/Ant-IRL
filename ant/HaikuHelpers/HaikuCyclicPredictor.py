...
from HaikuHelpers.HaikuActorCritic import HaikuContinuousActorCritic
...
def get_full_action_from_sample(predictor, angle, standard_deviation):
    ...
    lf_rb_action_prob = np.prod(
        normal_density(lf_rb_action, standard_deviation, lf_rb_action_mean)
    )
    rf_lb_action_prob = np.prod(
        normal_density(rf_lb_action, standard_deviation, rf_lb_action_mean)
    )
    return (
        lf_rb_action.flatten(),
        lf_rb_action_prob,
        rf_lb_action.flatten(),
        rf_lb_action_prob,
        action,
    )
...
class HaikuCyclicActorCritic(HaikuContinuousActorCritic):
    ...
    def train_actor_and_critic_on_batch(self, batch_size):
        ...
        value = (
            batch_data["rewards"]
            + self.discount * self.value_critic.evaluate(next_state).flatten()
        )
        advantage = value - self.value_critic.evaluate(current_state).flatten()

        action_means = self.actor.evaluate(current_state)
        current_policy_probabilties = np.prod(
            normal_density(action_means, self.noise_std, batch_data["actions"]), axis=1
        )
        importance_weight = np.minimum(
            5, np.divide(current_policy_probabilties, batch_data["action_probability"])
        )
        ...
        self.value_critic.params, self.value_critic.optimizer_state, critic_loss = self._train_weighted_critic(
            self.value_critic.params,
            current_state,
            value,
            importance_weight,
            self.value_critic.optimizer_state,
        )
        self.value_critic.generations += 1

        return actor_loss, critic_loss

    def _weighted_critic_loss(self, critic_params, states, values, weights):
    ...
    def _train_weighted_critic(
        ...
        loss, gradient = jax.value_and_grad(self._weighted_critic_loss)(
            critic_params, states, values, weights
        )
        update, new_optimizer_state = self.value_critic.optimizer.update(
    ...
    def run_episode_with_actor(self, env, max_len=100):
        ...
        for t in range(max_len):
            ...
            phase_action, phase_probility, anti_phase_action, anti_phase_probility, full_action = get_full_action_from_sample(
                self.actor, self.get_angle(), self.noise_std
            )

            _state_next, reward, terminal, info = env.step(full_action)

            self.update_training_queue(
                self.phase, reward, phase_action, phase_probility
            )
            self.update_training_queue(
                (self.phase + int(self.phase_steps / 2)) % self.phase_steps,
                reward,
                anti_phase_action,
                anti_phase_probility,
            )
            self.step_phase()
            if terminal:
                break
...
class AntCyclicEnvironment(AntIRLEnvironment):
    ...
    def __init__(
        self, port="/dev/ttyUSB0", speed=0.5, window_name="Ant Actor Critic Simplified"
    ):
        super().__init__(port, speed, window_name, sensors_enabled=False)
...
if __name__ == "__main__":
    ...
    path = "Environment/training_configs/fixed_cycle_configs/fixed_cycle_acc_config.yaml"
    ...
    input_dataset = WalkCycle(
        "walk_cycle_configs/nn_training_walk_config.yaml", speed=0.3
    ).get_all_frames()

    phase = 0
    env = AntCyclicEnvironment()
    data = {"actor_loss": [], "critic_loss": [], "success": [], "episode_length": []}
    try:
        for generation in range(1000):
            ...
            if generation % 5 == 0:
                print(
                    "Generation {} took {} frames".format(
                        generation, data["episode_length"][-1]
                    )
                )
        ...
        print("saving data...")
        file_name = f"Environment/training_configs/fixed_frame_data.p"
