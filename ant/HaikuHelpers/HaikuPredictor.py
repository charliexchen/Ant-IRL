...
from HaikuHelpers.JaxUtils import squared_loss, gen_mlp_from_config
...
class HaikuPredictor:
    ...
    def save_params(self, file_name=None):
        if file_name is None:
            if not os.path.exists(f"training_configs/{self.name}"):
                os.makedirs(f"training_configs/{self.name}")
            file_name = f"../training_configs/{self.name}/params_{self.generations}_gen.p"
        pickle.dump(self.params, open(file_name, "wb"))

    def get_params(self, file_name=None):
        if file_name is None:
            # gets the latest snapshot if not specified
            files_names = os.listdir(f"training_configs/{self.name}/")
...
if __name__ == "__main__":
    ...
    path = "Environment/training_configs/fixed_trainer_config.yaml"
